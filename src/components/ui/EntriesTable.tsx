...
export function EntriesTable({
  ...
  return (
    <div className="bg-white border border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] overflow-hidden flex flex-col h-full ring-0">
      {/* Header */}
      <div className="px-6 py-4 border-b border-black bg-white flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <h3 className="font-bold text-neutral-900 tracking-tight">{title}</h3>
          <span className="px-3 py-1 bg-black text-white text-[10px] font-bold uppercase tracking-normal border border-black">
            {data.length} TOTAL
          </span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <ToggleGroup.Root
            ...
            className="flex bg-white p-1 border border-black"
          ...
          <div className="flex items-center gap-2">
            ...
            {onProcessAll && (
              <Button
                ...
                className="gap-2 font-bold bg-primary hover:bg-primary/90 h-8 shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-y-[1px]"
              >
      ...
      {/* Filters (Expandable) */}
      {showFilters && (
        <div className="px-6 py-4 bg-neutral-50 border-b border-black animate-in slide-in-from-top-2 duration-200">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-400" />
            <Input
              type="text"
              placeholder="Buscar por cualquier campo..."
              className="pl-10 pr-10 py-2 h-9 border border-black focus-visible:ring-0 shadow-none bg-white font-mono"
              value={searchQuery}
        ...
        <ScrollArea.Viewport className="h-full w-full rounded-none">
          {isLoading ? (
            ...
            <table className="w-full text-left border-collapse min-w-[600px]">
              <thead className="sticky top-0 bg-white z-10 border-b border-black shadow-none">
                <tr className="text-[12px] font-bold text-black uppercase tracking-normal border-b border-black">
                  {columns.map((col, idx) => (
      ...
      {/* Footer Info */}
      <div className="px-6 py-2.5 bg-neutral-50 border-t border-neutral-100 flex items-center justify-between text-[10px] text-neutral-400 font-extrabold uppercase tracking-normal shadow-inner">
        <div className="flex items-center gap-2">
          {searchQuery ? (
